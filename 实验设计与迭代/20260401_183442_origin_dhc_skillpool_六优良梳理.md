...
当前这轮里，`107` 个成功题就是后续可用的 task-local source skill。  
这批 source skill 对应：
...
##### 7.3.5.5 基于这 `6` 个聚合 skill 的后续测试结果
