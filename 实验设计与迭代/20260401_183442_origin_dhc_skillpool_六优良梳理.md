...
问：那正式可比较的测试，其实是哪 `5` 组？

答：如果只保留**同口径、可正式横向比较**的结果，就是下面这 `5` 组。
...
| ----- | ----------- | ------------------------- | ------------: | ---------: |
| `1` | `gpt-5.2` | `Codex` 手工聚合        |        `62` | `0.4429` |
| `2` | `gpt-5.4` | `v5` 主线聚合库         |        `59` | `0.4214` |
| `3` | `gpt-5.2` | 真实 `gpt-5.2 API` 聚合 |        `57` | `0.4071` |
| `4` | `gpt-5.2` | 外部 `gpt-5.4 API` 聚合 |        `55` | `0.3929` |
| `5` | `gpt-5.4` | 外部 `gpt-5.4 API` 聚合 |        `54` | `0.3857` |

问：那你说的“辅助测试”是哪两组？为什么不算正式可比结果？

答：是下面这两组。它们是正式流程里的中间产物，但不该拿来和上面的 `5` 组并排当最终分数。
...
- **正式可比测试共有 `5` 组**：`7.3.5 baseline` + `7.3.5 gpt-5.4 API 聚合` + `7.5 Codex 聚合` + `7.5 gpt-5.4 API 聚合` + `7.5 gpt-5.2 API 聚合`；
