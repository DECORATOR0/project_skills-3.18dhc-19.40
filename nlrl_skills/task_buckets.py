...
from agent.skill_eval.skill_router import route_skill
...
def runtime_family_for_task(task: DatasetTask) -> str:
    question_id = str(task.metadata.get("original_question_id", task.task_id))
    return route_skill(question_id, task.prompt, task.file_list).skill_id
...
def classify_task_bucket(task: DatasetTask) -> TaskBucketInfo:
    ...
    if family == "earth-spectrum-drought-stress":
        bucket = "drought-tvdi"
        if re.search(r"\bati\b", question) or "thermal inertia" in question:
            bucket = "ati"
    elif family == "earth-spectrum-thermal-retrieval":
        bucket = "thermal-retrieval"
    elif family == "earth-product-raster-arithmetic":
        bucket = "arithmetic"
    elif family in {"earth-product-derived-index-change", "earth-product-timeseries"}:
        if any(token in question for token in ("hotspot", "fire", "burned", "burn scar", "fire-prone")):
            bucket = "hotspot-fire"
        elif family == "earth-product-derived-index-change":
